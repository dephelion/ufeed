...
- No competing on model accuracy with cloud LLMs. Compete on privacy, size (~33MB), predictability.
...
- State limits plainly: English only, words not images, subjects not quality, not perfect.
- Name no competitor in user-facing text.
- One short description, three places, identical: `wxt.config.ts` manifest, `package.json`, `docs/store-listing.md`. Store limit 132 chars.
