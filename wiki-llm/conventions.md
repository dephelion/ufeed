...
1. **Privacy.** Post text never leaves the device, never reaches a log, never persists. Zero analytics. A new Chrome install opens the welcome page; Firefox does not. The browser sends that page request to ufeed.es. Model weights come from the selected CDN. Neither request contains feed text, topics or extension settings. Embeddings of posts the reader explicitly rated persist in `storage.local` — vectors only, never text, never transmitted. See [privacy.md](privacy.md).
...
3. **Host page integrity.** All injected CSS namespaced `.lx-*`. No layout side effects beyond the documented `position: relative`. Never mutate a host element beyond class and `aria-hidden` toggles, namespaced `data-lx-*` attributes, and namespaced inline `--lx-*` custom properties — inert, since they paint nothing until one of our own rules reads them ([ui.md](ui.md) §The collapse). Our own nodes (engine iframe, thumbs bar, no-topics card, posts-hidden badge, `.lx-sr` announcer) carry `.lx-*` and are never inserted into a post.
...
| `error` | A uFeed bug: an invariant broke, a loaded model failed to embed | every build, `console.error` |
