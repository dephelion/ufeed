...
`score 0.793 / needs 0.795 · 105 chars · Topic #2 · disliked post` on every scored post, from `data-lx-*` attributes stamped by the content script (`src/feed/score-badge.ts`). `Topic #` is the 1-based position of the line that gave the score, absent when the post has no line (unscored). `needs` is the strictness threshold. The last field appears only when a near-identical rated post decided the verdict (not when a keep/blur word, the media or language rule, or a kept conversation did), and the colour follows that verdict, not the score.
