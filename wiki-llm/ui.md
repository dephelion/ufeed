...
- An opened post keeps a small tag with the reason, minus "click to read" (`data-lx-opened` + `data-lx-label`, `opened*` keys). Top-left and click-through: top-right sits on the host's menu buttons once the post shows. `revealAll()` removes it with the blurs.
