...
Fail-open everywhere. Unknown score, request timeout (8s), engine `ERROR`, or worker crash all **reveal**. A 15s watchdog logs (debug builds) if the engine never reports in. An unhandled error in the content script, worker, engine frame or offscreen page becomes `STATUS error`; never leave the last download percentage on screen or let a later ready status mask a fatal error. An error status resolves outstanding scores empty and rejects later scores until recovery, so a late reply cannot blur a revealed post. The worker keeps the last `SET_TOPICS` past a failed load and embeds it on the next request; a `SCORE` with no topics replies `ERROR`, never a score against nothing. No path may leave a post blurred because something broke.
