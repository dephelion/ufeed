...
content → engine   { id, type: 'FEEDBACK',   text, liked }
                   SET_TOPICS also carries each line's liked/disliked vectors and the blacklist lines
engine  → content  { id, type: 'SCORES',     scores, topics, lines, ratings, blocks }   raw cosine, never booleans; per score: best line index, every line's cosine, near-identical rating (true/false/null), best blacklist cosine (-1 when none)
