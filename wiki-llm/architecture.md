...
6. Re-read each returned container's text; ignore scores for reused nodes whose text changed. `decide()` blurs or reveals; the score is cached by text hash.
...
**Engine recovery deletes `transformers-cache`, then calls `runtime.reload()`; keep both storage keys.** Stored settings still pass through `withDefaults()`; invalid fields fall back individually. Stop and report a cache deletion error instead of claiming a clean retry.
...
Fail-open everywhere. Unknown score, request timeout (8s), engine `ERROR`, or worker crash all **reveal**. A 15s watchdog logs (debug builds) if the engine never reports in. Content startup failures and extension-origin script errors, plus unhandled errors in the worker, engine frame or offscreen page, become `STATUS error`; never leave the last download percentage on screen or let a later ready status mask a fatal error. Do not treat a feed page's `unhandledrejection` as an extension failure: the content script shares its window's event stream with the host, and Reddit's failed Sentry import triggered a false `Failed` state. Handle content-owned promises at their call sites. An error status resolves outstanding scores empty and rejects later scores until recovery, so a late reply cannot blur a revealed post. The worker keeps the last `SET_TOPICS` past a failed load and embeds it on the next request; a `SCORE` with no topics replies `ERROR`, never a score against nothing. No path may leave a post blurred because something broke.
