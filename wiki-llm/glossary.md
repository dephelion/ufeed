...
uFeed's task — a short topic against a longer post — is asymmetric, which is why a retrieval model wins. Using one without its prefixes, or a symmetric model with prefixes, gives meaningless scores.
