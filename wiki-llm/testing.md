...
**First run downloads both models** when `test:model` is run: ~197MB for Gemma and ~33MB for e5. CI runs `npm run check` only, so it never pays that.
...
How a translation fits. Every locale is checked for keys, placeholders, length and a place in the popup's language menu, never for wrapping or width; open the popup and a feed in each new language ([i18n.md](i18n.md)).
...
On a feed, a host-page `unhandledrejection` must leave engine status unchanged; a content-script error with an extension filename must report `Failed`. Reddit's failed Sentry import exposed the false-positive path.
