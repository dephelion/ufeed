...
- **Fail-open.** A slow or broken model must never leave the user staring at a
  blurred wall. Degradation means _less filtering_, never _less usable_.
...
forward pass _per label per text_, scales linearly in topic count, and produces
...
- _Name the subject in a few words._ Short noun lists (`tech, software, ai`)
...
Blur the _text subtree_ instead:
...
- **Click / tap:** reveals the post and it stays revealed. The click is consumed
  and does _not_ reach the post underneath, so it never navigates.
...
- Check `node.matches(sel)` **as well as** `node.querySelectorAll(sel)` — an
  added node that _is_ a post is otherwise missed entirely.
