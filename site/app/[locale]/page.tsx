    ...
    <main>
      ...
      <section className="hero wrap">
        <div className="hero-copy">
          <span className="eyebrow">
            <i /> {es ? 'TU FEED, A TU MANERA' : 'YOUR FEED, YOUR WAY'}
          </span>
          <h1>{es ? 'Un poco más de lo que buscas.' : 'A little more of what you came for.'}</h1>
          <p>
            {es ? 'uFeed es una extensión gratuita para el navegador que difumina las publicaciones que no coinciden con tus intereses, para que encuentres más fácilmente las que sí.' : 'uFeed is a free browser extension that gently blurs posts outside your interests, so it’s easier to find the ones that are.'}
          </p>
          <div className="actions">
            ...
            >
              <ChromeIcon /> {es ? 'Añadir a Chrome' : 'Add to Chrome'} <span>↗</span>
            </a>
            <a
              ...
              rel="noopener noreferrer"
            >
              <GitHubIcon /> {es ? 'Explorar el proyecto' : 'Explore the project'}
            </a>
          </div>
          <div className="quiet-note">
            <span>✳</span> {es ? 'Gratis para siempre · Código abierto · Tus datos son tuyos' : 'Free, now and always · Open source · Your data stays yours'}
          </div>
        </div>
        <ReelPreview />
      </section>
      ...
      <section className="why wrap">
        <div className="section-heading">
          <span className="eyebrow">{es ? 'UN FILTRO DIFERENTE' : 'A BETTER KIND OF FILTER'}</span>
          <h2>{es ? 'Por qué uFeed' : 'Why uFeed'}</h2>
          <p>{es ? 'Empieza por lo que quieres ver más. Deja que lo demás pase a un segundo plano.' : 'Start with what you want more of. Let the rest fade into the background.'}</p>
        </div>
      ...
      <section className="closing wrap">
        <p>{es ? 'Menos ruido. Más de lo que te gusta.' : 'Less noise. More of your thing.'}</p>
        <a href={CHROME_STORE_URL} target="_blank" rel="noopener noreferrer">
          {es ? 'Descubre uFeed en Chrome Web Store' : 'See uFeed in the Chrome Web Store'} <span>↗</span>
        </a>
      </section>
    </main>
  );
